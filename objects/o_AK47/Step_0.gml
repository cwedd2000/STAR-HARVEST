...
if mouse_check_button(mb_left) and canshoot and instance_exists(o_AK47) and o_play.iron>3{
	image_speed=1;o_play.iron-=3;
	canshoot=false;
	alarm[0]=7;

	instance_create_layer(shell1.x+lengthdir_x(-11,dir),shell1.y+lengthdir_y(-11,dir),"instances",o_bulletsa);
}
if (mouse_check_button(mb_right) or keyboard_check(ord("F"))) and cangoo and instance_exists(o_AK47) and o_play.iron>5{
	image_speed=1;o_play.iron-=5;
	cangoo=false;
	alarm[1]=20;
var shot=instance_create_layer(shell1.x+lengthdir_x(-11,dir),shell1.y+lengthdir_y(-11,dir),"instances",o_bulletsa1);
